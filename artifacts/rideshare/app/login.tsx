...
function RoleCard({ label, description, icon, active, onPress }: { label: string; description: string; icon: React.ComponentProps<typeof MaterialCommunityIcons>["name"]; active: boolean; onPress: () => void; }) {
  const colors = useColors();
  return <Pressable onPress={onPress} style={[styles.roleCard, { backgroundColor: active ? colors.foreground : colors.card }]}><MaterialCommunityIcons name={icon} size={20} color={active ? colors.background : colors.foreground} /></Pressable>;
}

const styles = StyleSheet.create({ root: { flex: 1 }, headline: { fontSize: 34, fontWeight: "800" }, sub: { marginTop: 8, marginBottom: 18 }, fieldLabel: { marginTop: 14, marginBottom: 8 }, roleRow: { flexDirection: "row", gap: 12 }, roleCard: { flex: 1, padding: 18, borderRadius: 18 }, input: { flexDirection: "row", alignItems: "center", gap: 10, borderWidth: 1, borderRadius: 14, paddingHorizontal: 14, paddingVertical: 12, marginBottom: 10 }, inputTxt: { flex: 1 }, error: { marginTop: 6, marginBottom: 10 }, footer: { marginTop: 18, textAlign: "center" } });
